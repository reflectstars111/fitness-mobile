...
新增首页“导入视频 · 回放分析”：复用姿态引擎，按真实视频时间检查局部问题、浏览骨骼和角度、逐项备注并导出。通用器械推胸无需品牌/型号，固定机位下可形成局部躯干变化候选。详见 [回放说明](../docs/video-replay.md) 与 [0.3 验证](../docs/validation-v0.3.md)。推胸尚未加入相机实时训练菜单。
